...
impl Input {
    ...
    fn iterate(&mut self) {
        let copy: Vec<Vec<char>> = self.matrix.clone();

        let collect_adjacent = |r: &usize, c: &usize| {
            let r: isize = *r as isize;
            let c: isize = *c as isize;
            let mut neighbours: Vec<char> = Vec::new();
            for rr in r - 1..r + 2 {
                for cc in c - 1..c + 2 {
                    if rr < 0 || rr >= copy.len() as isize {
                        continue;
                    }
                    if cc < 0 || cc >= copy[r as usize].len().try_into().unwrap() {
                        continue;
                    }
                    // ignore self
                    if r == rr && c == cc {
                    ...
                    neighbours.push(copy[rr as usize][cc as usize])
                }
            }
        ...
        for row in 0..self.matrix.len() {
            for col in 0..self.matrix[row].len() {
                self.matrix[row][col] = match copy[row][col] {
                    '.' => '.',
                    'L' => {
                        if collect_adjacent(&row, &col)
                            .iter()
                            .filter(|c| **c == '#')
                            .count()
                            == 0
                        {
                            self.is_fixpoint = false;
                            '#'
                        } else {
                            'L'
                        }
    ...
    fn count(&self) -> u64 {
        self.matrix.iter().flatten().filter(|c| c == &&'#').count() as u64
    }
}

pub fn run(inputs: &[&str]) -> Result<u64, &'static str> {
    let mut prog = Input::new(inputs);
    while !prog.is_fixpoint {
        prog.iterate();
        // println!(
        //     "{:?}",
        //     &prog
...
mod tests {
    ...
    fn part1_works_for_sample_input() {
        assert_eq!(run(&SAMPLE_INPUT).unwrap(), 37, "wrong puzzle solution");
    }

    #[test]
    fn part1_works_for_puzzle_input() {
        assert_eq!(run(&PUZZLE_INPUT).unwrap(), 2289, "wrong puzzle solution");
    }
}
